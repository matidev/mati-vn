...
jQuery(function ($) {
    ...
    $('.post-content').find('h2, h3, h4, h5, h6').filter('[id]').each(function () {
        $(this).html('<a href="#' + $(this).attr('id') + '" class="heading-anchor"><span><i class="fa-solid fa-link"></i></span> ' + $(this).text() + '</a>');
    });
    ...
    if ($cctvBlock.length) {
        const
            ...
            loading = '<i class="fa-solid fa-spinner fa-spin-pulse text-primary"></i>'
        ;
        ...
        let
            ...
            $cctvMorePortsChbox = $cctvForm.find('#more-ports'),
            ...
            mToken = '',
            getListPort = function () {
                if ($cctvMorePortsChbox.is(':checked')) {
                    return [80, 443, 8080, 8000, 554, 34567, 3389, 3399, 21, 22, 3306, 5900, 7777];
                } else {
                    return [80, 8000, 8080, 554, 443];
                }
            },
            checkOpenPort = function (idx) {
                ...
                if (typeof ip !== 'undefined' && ip.length > 0 && typeof port !== 'undefined') {
                    ...
                    $.ajax({
                        ...
                        if (res.success && parseInt(res.data.portStatus) === 1) {
                            $cctvResultList.append('<li class="list-group-item font-weight-bold">' +
                                '<a href="http://' + ipPort + '" target="_blank" class="">' + ipPort + '</a>' +
                                '<span class="float-right text-success">Opened <i class="fa-solid fa-circle-check"></i></span></li>');
                        } else {
                            $cctvResultList.append('<li class="list-group-item">' + ipPort +
                                '<span class="float-right text-danger">Closed <i class="fa-solid fa-circle-xmark"></i></span></li>');
                        }
                    ...
                    $cctvMorePortsChbox.attr('disabled', false);
                }
            },
            getIpAddress = function (ip) {
                ...
                $cctvMorePortsChbox.attr('disabled', true);
                NProgress.start();
                $cctvProgressBar.addClass('progress-bar-striped progress-bar-animated')
        ...
        $toolsInput.on('input', function () {
            ...
            if (typeof value !== 'undefined' && value !== '') {
                ...
                if (typeof CryptoJS !== 'undefined') {
                    ...
                    if (typeof CryptoJS.SHA1(value) !== 'undefined') {
                        $toolsResult.append(toolResultTpl('SHA-1 Hash', CryptoJS.SHA1(value), true));
                    }
                }
    ...
    if ($cvBlock.length) {
        let
            ...
            html = '<div class="alert alert-success d-lg-inline-block" role="alert">\n' +
                ...
                '    <p>Unfortunately, the CV does not exist or the token has expired.</p>\n' +
                '</div>';
        ;
        ...
        if (typeof token !== 'undefined' && token !== '' && token.length > 1 && token.length <= 32) {
            ...
            $.ajax({
                ...
                NProgress.done();
                $cvBlock.html(res.success ? res.data.html : html);
            }).fail(function (jqXHR, status) {
                NProgress.done();
                $cvBlock.html(html);
            });
